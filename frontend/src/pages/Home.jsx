...
const Home = () => {
    ...
    useEffect(() => {
        const fetchWebsites = async () => {
            try {
                // Fetch latest websites (not just featured) to show new submissions immediately
                const response = await getWebsites({ sort: 'newest', limit: 6 });
                if (response.success) {
                    setFeaturedWebsites(response.websites || []);
                }
            } catch (error) {
    ...
    return (
        <PageLayout>
            <div className="overflow-hidden">
                ...
                <section className="relative min-h-screen flex items-center pt-32 pb-16 sm:pt-36 sm:pb-20 lg:pt-24 lg:pb-0 overflow-hidden bg-[#FAFBFF]">
                    {/* Background Elements */}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {loading ? (
                                ...
                                featuredWebsites.map(website => (
                                    <WebsiteCard key={website._id || website.id} website={website} viewMode="grid" />
                                ))
                            ) : (
                                <div className="col-span-full text-center py-12 text-gray-400 font-medium">
                                    No featured websites found at the moment.
                                </div>
                            )}
                        </div>
